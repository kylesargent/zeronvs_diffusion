...
https://github.com/Lightning-AI/lightning/tree/b845c449a89f72c78570462aa1a0d05d2f91a6b4/pytorch_lightning
"""
...
from ldm.models.diffusion import sampling_util
...
def _log_images(image_dict, directory):
    ...
    # pil_image_dict = {k:to_img(v) for (k,v) in image_dict.items()}

    for k, v in image_dict.items():
        image = to_img(v)
        with open(os.path.join(directory, k + ".png"), "wb") as fp:
            image.save(fp)
...
def log_memory_summary():
    ...
    # return sizes
    numels = sum(size.numel() for size in sizes)
    n_tensors = len(sizes)
    print()


def get_current_sizes():
    current_sizes = collections.Counter()
    ...
    for obj in gc.get_objects():
        try:
            if torch.is_tensor(obj) or (
                ...
                size = obj.size()
                current_sizes.update([repr(size)])
                size_list.append(size)
        except:
            pass

    numels = sum(size.numel() for size in size_list)
    n_tensors = len(size_list)

    return current_sizes, numels, n_tensors


SIZES, _, _ = get_current_sizes()


def log_new_tensors(message):
    global SIZES
    current_sizes, numels, n_tensors = get_current_sizes()

    diff = current_sizes - SIZES
    SIZES = current_sizes

    message += f": {n_tensors} tensors llocated, {numels/1e9} million total elts"
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        self.batch_uid_statistics = collections.Counter()
        self.batch_pair_uid_statistics = collections.Counter()
        ...
        if ckpt_path is not None:
            # ckpt restoration should come from either
    ...
    def get_input_tensor(self, batch, k):
        x = batch[k]
        if len(x.shape) == 3:
            x = x[..., None]
    ...
    def get_input(self, batch, k):
        return self.get_input_tensor(batch, k)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.batch_uid_statistics.update(batch["uid"])
        [(_, n_most_frequent_scene)] = self.batch_uid_statistics.most_common(1)
        n_unique_scenes = len(self.batch_uid_statistics)
        self.log(
            "n_most_frequent_scene",
            n_most_frequent_scene,
            prog_bar=True,
            logger=True,
            on_step=True,
        ...
        self.batch_pair_uid_statistics.update(batch["pair_uid"])
        [(_, n_most_frequent_pair)] = self.batch_pair_uid_statistics.most_common(1)
        n_unique_pairs = len(self.batch_pair_uid_statistics)
        self.log(
            "n_most_frequent_pair",
            n_most_frequent_pair,
            prog_bar=True,
            logger=True,
            on_step=True,
    ...
    @torch.no_grad()
    def shared_valtest_step(self, batch, batch_idx):
        # return {
        #     "psnr": 0., "ssim": 0., 'lpips': 0.
        # }
        # log_new_tensors("before valtest step")
        ...
        gt_target_im = batch["image_target"] / 2 + 0.5
        ...
        lpips_metric = (
            lpips_fn(pred_target_im * 2 - 1, gt_target_im.permute((0, 3, 1, 2)) * 2 - 1)
            .cpu()
            .numpy()
        )
        # print(lpips_metric)
        ...
        gt_target_im = gt_target_im.cpu().numpy().transpose((0, 3, 1, 2))

        # psnr =
        image_dict = {
            "gt_input_im": input_im.cpu().numpy() / 2 + 0.5,
            "gt_target_im": gt_target_im,
            "pred_target_im": pred_target_im,
        }
        ...
        ssims = [
            structural_similarity(
                gt_target_im[i], pred_target_im[i], data_range=1.0, channel_axis=0
            )
            for i in range(gt_target_im.shape[0])
        ]
        ssim = np.mean(ssims)
